...
module.exports = (client) => {
  client.on('guildMemberRemove', async (member) => {
    const servers = getServers().filter(s => s.posted && s.userId === member.id);
    const pending = getPending().filter(e => e.userId === member.id);
    if (servers.length === 0 && pending.length === 0) return;
    const ids = servers.map(s => s.id);
    for (const e of pending) removePending(e.id);
    ...
    saveServers(all.filter(s => !ids.includes(s.id)));
  });
  ...
  client.once('clientReady', async () => {
    ...
    const runCycle = async () => {
      ...
      let servers = getServers().filter(s => s.posted);
      const toRemove = [];
      ...
      if (toRemove.length > 0) {
        const all = getServers();
        saveServers(all.filter(s => !toRemove.includes(s.id)));
