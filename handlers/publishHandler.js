...
const fs = require('fs');
const { getConfig, addPending, removePending, addServer, getPending, getServers, saveServers, getUI } = require('../utils/storage');
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('publish_modal_')) {
      ...
      const entry = {
        ...
        type,
        inviteLink: interaction.fields.getTextInputValue('invite_link'),
      ...
      await interaction.reply({ content: ui.messages.publish.submitted, flags: MessageFlags.Ephemeral });
      return;
    }
