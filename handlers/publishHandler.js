...
async function getOrCreateWebhook(client, section) {
  ...
  const existing = config.webhooks?.[section];
      ...
      if (guildId) {
        ...
        if (oldPending || oldServer) {
          ...
          for (const e of allPending) {
            if (!e.guildId) {
              try { const inv = await client.fetchInvite(e.inviteLink); if (inv?.guild?.id === guildId) { e.guildId = guildId; await savePending(allPending); } } catch {}
            }
          }
          for (const s of allServers) {
            if (!s.guildId) {
              try { const inv = await client.fetchInvite(s.inviteLink); if (inv?.guild?.id === guildId) { s.guildId = guildId; await saveServers(allServers); } } catch {}
            }
          }
        }
      }
    ...
    if (interaction.isButton() && interaction.customId.startsWith('publish_')) {
      ...
      if (action === 'approve') {
        ...
        if (msg) {
          ...
          if (srv) { srv.messageId = msg.id; srv.channelId = msg.channelId || channelId; await saveServers(servers); }
        }
