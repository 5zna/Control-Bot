  ...
  menus: {
    download: {
      ...
      options: {
        ...
        instagram: { label: 'Instagram', description: 'تحميل فيديوهات انستغرام', emoji: '📸' },
        twitter: { label: 'Twitter / X', description: 'تحميل فيديوهات تويتر', emoji: '🐦' },
        facebook: { label: 'Facebook', description: 'تحميل فيديوهات فيسبوك', emoji: '📘' },
      },
    },
    publish: {
...
function savePending(entries) {
  writeJson(PENDING_PATH, entries);
}

function getServers() {
  return readJson(SERVERS_PATH, []);
}

function saveServers(entries) {
  writeJson(SERVERS_PATH, entries);
}
...
function removePending(id) {
  const entries = getPending();
  savePending(entries.filter(e => e.id !== id));
}

function addServer(entry) {
  const entries = getServers();
  entries.push(entry);
  saveServers(entries);
}
